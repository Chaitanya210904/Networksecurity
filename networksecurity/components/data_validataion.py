...
class DataValidation:
    ...
    def is_numerical_column_exist(self, dataframe: pd.DataFrame) -> bool:
        try:
            ...
            if missing_columns:
                logging.warning(f"Missing numerical columns: {missing_columns}")
                return False
    ...
    def detect_dataset_drift(self, base_df, current_df, threshold=0.05) -> bool:
        try:
            ...
            return status  # It's good practice to return status here
        except Exception as e:
            raise NetworkSecurityException(e, sys)
    ...
    def initiate_data_validation(self) -> DataValidationArtifact:
        try:
            ...
            if not status:
                error_message = f"{error_message}train dataframe is missing some numerical columns.\n"

            status = self.is_numerical_column_exist(dataframe=test_dataframe)
            if not status:
                error_message = f"{error_message}test dataframe is missing some numerical columns.\n"
